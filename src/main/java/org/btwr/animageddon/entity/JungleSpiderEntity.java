...
public class JungleSpiderEntity extends SpiderEntity {
    ...
    public static boolean canSpawn(EntityType<JungleSpiderEntity> type,
        ...
        boolean onLeaves = below.isIn(BlockTags.LEAVES);

        boolean validGround = below.isSolidBlock(world, pos.down());
    ...
    static class LeafWanderGoal extends WanderAroundFarGoal {
        public LeafWanderGoal(JungleSpiderEntity spider, double speed) {
            super(spider, speed);
        }
    ...
    public boolean isInAttackRange(LivingEntity target) {
        ...
        if (this.getWorld().getBlockState(below).isIn(BlockTags.LEAVES)) {
            // If target is more than 2 blocks below, consider them in range
