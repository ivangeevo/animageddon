## Changelog Animageddon
# v???(dev)
+ Changed cows to be milked only once a day and to initially spawn without milk. This periodic milking change will remain until the change for animals requiring food is implemented into the mod
+ Fixed a bug with chickens being able to be fed again after a short amount of time when they have already been fed in this egg laying cycle
+ Updated all mod items in the creative menu tabs to be positioned after similar items instead of being at the bottom of the list

# v0.2.1
+ Fixed missing recipe for Chicken Feed.
+ Updated the mod to Fabric API 0.116.7, Fabric Loader 0.17.2 & BTWR: Shared Library 0.61

# v0.2
+ Added chickens requiring chicken feed to lay eggs and with this breeding has been disabled.
+ Updated the mod to Fabric API 0.116.0, Fabric Loader 0.16.14 & BTWR: Shared Library 0.54

# v0.1.1
+ Fixed a bug where creepers would drop only 1 nitre on death instead of maximum 2 as it does gunpowder normally
+ Updated the mod to Fabric API 0.115.0, Fabric Loader 0.16.10 & BTWR Shared Library 0.47

# v0.1
+ Initial Release